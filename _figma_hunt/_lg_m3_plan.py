"""M3 Design Kit 真实 fileKey → FileByKey + PlanByFileKey(Google org 计费数据)"""
import sys, json, asyncio
import websockets
sys.stdout.reconfigure(encoding="utf-8", errors="replace")
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/151.0.0.0 Safari/537.36"
PUB = "CYs4jJGyYeUxpAVcJ2EAZ4"  # Material 3 Design Kit (真实 fileKey,来自 Notion 公开链接)
...
async def main():
    await sub("FileByKey→M3", "FileByKey", {"fileKey": PUB})
    await sub("PlanByFileKey→M3", "PlanByFileKey", {"fileKey": PUB}, wait=20)
    await sub("LibraryFileSubscriptions→M3", "LibraryFileSubscriptions", {"fileKey": PUB})
