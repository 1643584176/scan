"""Plan 视图家族完整测试：PlanByFileKey(完整输出) + PlanByOrgId + PlanByTeamId"""
...
async def main():
    await sub("PlanByFileKey→PUB(完整)", "PlanByFileKey", {"fileKey": PUB}, wait=20)
    await sub("PlanByOrgId→DemoOrg", "PlanByOrgId", {"orgId": ORG})
    await sub("PlanByTeamId→DemoTeam", "PlanByTeamId", {"teamId": TEAM})
