...
async def sub_view(label, args, cookie, client_url, wait=12):
    frames = []
    try:
        async with websockets.connect(lg_url(client_url),
            ...
            for _ in range(3):
                ...
                if isinstance(msg, str) and "authSuccess" in msg:
                    authed = True
                    break
            print(f"[{label}] auth={'OK' if authed else 'FAIL?'}")
...
ARGS_A = {"planId": "1666382706663462213", "planType": "team"}

asyncio.run(sub_view("A基线(刷新后)", ARGS_A, CK_A, "5Gs4PaTz11Hlk2sqVnidBG"))
asyncio.run(sub_view("B→A(对照)", ARGS_A, CK_B, "5Gs4PaTz11Hlk2sqVnidBG"))
