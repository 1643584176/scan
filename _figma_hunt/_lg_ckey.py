...
def lg_url():
    return (f"wss://www.figma.com/api/livegraph?pv=1&pr=251c5be83e6853e5&pt=1786072093"
            ...
            f"&clientUrl=https%3A%2F%2Fwww.figma.com%2Ffile%2F5h8vKLfD3ItG0RxCGIUHuf"
...
async def sub(label, view_name, args, wait=12):
        ...
        async with websockets.connect(lg_url(),
            ...
            await ws.send(json.dumps({"messageType": "auth", "clientType": "web",
                                      ...
                                      "tags": {"clientType": "web",
                                               ...
                                               "clientUrl": "https://www.figma.com/file/5h8vKLfD3ItG0RxCGIUHuf"},
            ...
            await ws.send(json.dumps({"messageType": "subscribe", "viewName": view_name,
                                      "viewHash": "c" * 32, "loadType": "initial", "args": args}))
    ...
    for f in frames:
        if '"initial":' in f or 'viewSubscriptionFailed' in f or '"errors"' in f:
            print(f"    🖼 {f[:1200]}")
...
async def main():
    await sub("FileByKey 匿名→checkpoint", "FileByKey", {"fileKey": "5h8vKLfD3ItG0RxCGIUHuf"})
    await sub("PlanByFileKey 匿名→checkpoint", "PlanByFileKey", {"fileKey": "5h8vKLfD3ItG0RxCGIUHuf"})
