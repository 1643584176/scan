# -*- coding: utf-8 -*-
import io, sys, json
from playwright.sync_api import sync_playwright
sys.stdout.reconfigure(encoding='utf-8', errors='replace')
raw=io.open('ws_cookie_A_new.txt',encoding='utf-8').read().strip().replace('\n','; ')
pairs=[p.split('=',1) for p in raw.split('; ') if '=' in p]
cookies=[]
for k,v in pairs:
    c={'name':k,'value':v,'path':'/','secure':True,'sameSite':'Lax'}
    if not k.startswith('__Host-'):
        c['domain']='.figma.com'
    else:
        c['url']='https://www.figma.com'; del c['path']
    cookies.append(c)
with sync_playwright() as p:
    b=p.chromium.launch(headless=False)
    ctx=b.new_context(viewport={'width':1600,'height':900})
    ctx.add_cookies(cookies)
    page=ctx.new_page()
    page.goto('https://www.figma.com/files', wait_until='domcontentloaded', timeout=60000)
    page.wait_for_timeout(30000)
    info=page.evaluate('''() => {
      const out={buttons:[], title:document.title};
      document.querySelectorAll('button, a, [role="button"]').forEach(e=>{
        const t=(e.innerText||'').trim();
        if(t && t.length<60 && /new|design|create|draft/i.test(t)) out.buttons.push(t.slice(0,50));
      });
      return out;
    }''')
    print('TITLE:', info['title'])
    print('NEW按钮:', json.dumps(list(set(info['buttons']))[:15],ensure_ascii=False))
    # 尝试点击 New design file
    for txt in ['New design file','Design file','+ New design file','New file']:
        loc=page.get_by_text(txt, exact=True).first
        try:
            loc.click(timeout=3000)
            print('-> 点击', txt)
            break
        except Exception:
            continue
    page.wait_for_timeout(20000)
    print('URL:', page.url)
    info2=page.evaluate('''() => {
      const t=[];
      document.querySelectorAll('[class*="toolbelt_mode_segmented_control"] *').forEach(e=>{
        const x=e.textContent.trim();
        if(x&&x.length<20&&e.children.length===0){ const r=e.getBoundingClientRect(); t.push({x, w:Math.round(r.width), h:Math.round(r.height)}); }
      });
      return {title:document.title, toolbelt:t};
    }''')
    print('AFTER:', json.dumps(info2,ensure_ascii=False)[:400])
    page.screenshot(path='ui_newfile.png')
    b.close()
