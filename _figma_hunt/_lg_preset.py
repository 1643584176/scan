"""Preset 库测试：CommunityLibraryData/Modules → libraryKey → 后续 LibraryDataByLibraryKey"""
import sys, json, asyncio
import websockets
sys.stdout.reconfigure(encoding="utf-8", errors="replace")
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/151.0.0.0 Safari/537.36"
PRESETS = ["1380235722331273046", "1035203688168086460", "1385659531316001292"]
    ...
    try:
        async with websockets.connect(lg_url(),
            ...
            await ws.send(json.dumps({"messageType": "subscribe", "viewName": view_name,
                                      "viewHash": "d" * 32, "loadType": "initial", "args": args}))
...
async def main():
    for pid in PRESETS:
        await sub(f"CommunityLibraryData→{pid}", "CommunityLibraryData", {"hubFileId": pid})
        await sub(f"CommunityLibraryModules→{pid}", "CommunityLibraryModules", {"hubFileId": pid})
