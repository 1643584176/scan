"""FileByKey 匿名 → 公开文件：libraryKey/sourceFileKey/teamId/parentOrgId"""
...
async def sub(label, file_key, wait=12):
    frames = []
    try:
        async with websockets.connect(lg_url(file_key),
...
async def main():
    await sub("FileByKey 匿名→公开 bv2nMIdFf4u3dESGail4sm", "bv2nMIdFf4u3dESGail4sm")
