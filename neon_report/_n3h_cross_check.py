# -*- coding: utf-8 -*-
"""交叉验证:账号下项目列表(找第二个项目测 presign 是否全局挂)+ 清理 kb 残留"""
...
print('projects -> %d | %s' % (st, raw.decode(errors='replace')[:600]))
try:
    projects = json.loads(raw).get('projects', [])
except Exception:
    projects = []

# 2. 对每个项目的主分支测 presign(若 >1 项目)
for prj in projects:
    pid = prj['id']
...
# 3. 清理 orange-sun 残留 kb1/kb2/kb3
P = 'orange-sun-90493739'
B = 'br-wandering-field-w2ob6mpn'
for b in ('kb1', 'kb2', 'kb3'):
    st5, _ = req('DELETE', '/projects/%s/branches/%s/buckets/%s' % (P, B, b))
    print('clean %s -> %d' % (b, st5))
st6, raw6 = req('GET', '/projects/%s/branches/%s/buckets' % (P, B))
print('final buckets -> %d | %s' % (st6, raw6.decode(errors='replace')[:300]))
