# -*- coding: utf-8 -*-
"""RLS 矩阵 v4:插 key 后台等 25 分钟自然轮询,成功后自动跑矩阵"""
...
cur.execute('CREATE TABLE public.demo_rls (id serial PRIMARY KEY, secret text, owner_id text)')
cur.execute("INSERT INTO public.demo_rls (secret, owner_id) VALUES ('s1','a'), ('s2','b'), ('s3','c')")
cur.execute('ALTER TABLE public.demo_rls ENABLE ROW LEVEL SECURITY')
cur.execute("CREATE POLICY only_own ON public.demo_rls FOR SELECT USING (auth.user_id() = owner_id)")
...
now0 = int(time.time())
ready = False
for attempt in range(26):  # 25 分钟
    tok = make_jwt({'sub': 'a', 'role': 'authenticated', 'iat': now0, 'exp': now0 + 7200})
    st, raw = da('demo_rls?select=id,secret,owner_id', tok)
...
if ready:
    now = int(time.time())
    tests = [
        # (名称, sub, role)  —— policy: auth.user_id() = owner_id
        ('auth as a', {'sub': 'a', 'role': 'authenticated', 'iat': now, 'exp': now + 3600}),
        ('auth as b', {'sub': 'b', 'role': 'authenticated', 'iat': now, 'exp': now + 3600}),
        ('auth as x', {'sub': 'x', 'role': 'authenticated', 'iat': now, 'exp': now + 3600}),
        ('anon as a', {'sub': 'a', 'role': 'anonymous', 'iat': now, 'exp': now + 3600}),
        ('owner', {'sub': 'a', 'role': 'neondb_owner', 'iat': now, 'exp': now + 3600}),
        ('cloud_admin', {'sub': 'a', 'role': 'cloud_admin', 'iat': now, 'exp': now + 3600}),
        ('no role', {'sub': 'a', 'iat': now, 'exp': now + 3600}),
        ('no sub auth', {'role': 'authenticated', 'iat': now, 'exp': now + 3600}),
    ]
    for name, pl in tests:
        st, raw = da('demo_rls?select=id,secret,owner_id', make_jwt(pl))
        print('[MATRIX %s] -> %d | %s' % (name, st, raw.decode(errors='replace')[:250]), flush=True)
        time.sleep(1)
else:
    print('cache not refreshed in 25min', flush=True)
