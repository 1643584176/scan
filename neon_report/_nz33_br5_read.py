...
st, raw = req('/projects/%s/connection_uri?database_name=neondb&role_name=neondb_owner&branch_id=%s' % (P, BR5))
...
with psycopg.connect(uri, connect_timeout=20) as conn:
    ...
    cur = conn.cursor()
    cur.execute('select id, email from public.sbx_anon_src order by id')
    print('\n-- masked email values (executor identity):')
