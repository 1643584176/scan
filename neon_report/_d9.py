# -*- coding: utf-8 -*-
"""RLS 语义测试 v2:jwt_cache 调小后重测 + 还原"""
...
# 1) 缓存调小
st, raw = ctl_req('PATCH', '/projects/%s/branches/%s/data-api/neondb' % (P, B),
                  {'settings': {'jwt_cache_max_lifetime': 1}})
print('PATCH cache=1 ->', st, flush=True)
time.sleep(3)
...
for name, pl in tests:
    ...
    print('[%s] -> %d | %s' % (name, st, raw.decode(errors='replace')[:250]), flush=True)
    time.sleep(2)  # 等缓存过期
...
conn.close()
st, raw = ctl_req('PATCH', '/projects/%s/branches/%s/data-api/neondb' % (P, B),
                  {'settings': {'jwt_cache_max_lifetime': 300}})
print('restore cache=300 ->', st, flush=True)
