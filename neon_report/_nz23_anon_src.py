# -*- coding: utf-8 -*-
"""连 br3: 查 anon 解析函数源码(找 valid label 逻辑) + 实验 masking 语法"""
...
with psycopg.connect(uri, connect_timeout=20) as conn:
    ...
    cur = conn.cursor()
    # 找含 'not a valid label' 的函数
    cur.execute("""select n.nspname, p.proname, p.prosrc
        from pg_proc p join pg_namespace n on n.oid=p.pronamespace
        where p.prosrc like '%valid label%' or p.prosrc like '%not a valid%'""")
    print('-- funcs containing error text:')
    ...
    cur.execute("""select p.proname, pg_get_function_identity_arguments(p.oid)
        from pg_proc p join pg_namespace n on n.oid=p.pronamespace
        where n.nspname='anon' and (p.proname like '%add%' or p.proname like '%regist%'
          or p.proname like '%masking%' or p.proname like '%label%' or p.proname like '%direct%')
        order by 1""")
    print('\n-- anon admin funcs:')
    for row in cur.fetchall():
        print('  ', row)
