# -*- coding: utf-8 -*-
"""br3: 查 pg_trusted_functions 定义 + 修正 SECURITY LABEL 实验"""
...
with psycopg.connect(uri, connect_timeout=20) as conn:
    ...
    cur = conn.cursor()
    # 视图定义
    for v in ('pg_trusted_functions', 'pg_masking_rules', 'pg_masks'):
        cur.execute("select pg_get_viewdef(%s::regclass, true)", ('anon.%s' % v,))
        print('== anon.%s def:' % v)
        print(cur.fetchone()[0][:2500])
        print('---')
