...
"""
URL 分类分析模块
可以单独运行: python modules/url_analyzer.py <urls_file> <output_dir>
"""
import sys
import os
import subprocess
...
from core.config import get_tool_path, TIMEOUT_URL_ANALYZER
...
def analyze_urls(urls_file, output_dir='.'):
    ...
    log(f"开始 URL 分类分析: {urls_file}")
    
    try:
        tool_path = get_tool_path('url_analyzer')
        ...
        ], cwd=os.getcwd(), timeout=TIMEOUT_URL_ANALYZER)
        
        if result.returncode == 0:
            log("[✓] URL 分类分析完成")
            return True
        else:
            log(f"[!] URL 分类分析返回码 {result.returncode}")
            return False
...
if __name__ == '__main__':
    ...
    success = analyze_urls(urls_file, output_dir)
