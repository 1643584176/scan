...
hs = list(re.finditer(r'<(?:h1|h2|h3)[^>]*>(.*?)</(?:h1|h2|h3)>', b, re.S))[:6]
for m in hs:
    print('H:', re.sub(r'<[^>]+>', '', m.group(1)).strip()[:200])
for kw in ['Thanks', 'Congrat', 'success', 'error', 'Error', 'login', 'password', 'Website', 'Tracking', 'admin']:
    i = b.find(kw)
    if i >= 0:
        print(kw, 'at', i)
...
print('links:', sorted(set(re.findall(r'href="([^"]*)"', b)))[:12])
