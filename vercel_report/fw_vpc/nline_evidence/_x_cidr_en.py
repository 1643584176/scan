# -*- coding: utf-8 -*-
"""allowedCIDRs functionality check + custom empty-policy behavior (D-line attribution completion)
E1: custom {} (empty) -> curl httpbin.org (docs: behaves as deny-all)
E2: custom + allowedCIDRs=["8.8.8.0/24"] -> TCP DNS 8.8.8.8:53 data layer (is IP allowlist effective?)
E3: E2 with 1.1.1.1:53 control (expected unreachable)
E4: custom + allowedCIDRs=["8.8.8.0/24"] + deniedCIDRs=["8.8.8.0/24"] -> behavior on conflict
E5: custom + allowedCIDRs=["172.31.0.0/16"] -> PG 172.31.0.2:5432 (explicit allow of private range = expected?)
"""
...
ip = sys.argv[1]
# TCP DNS query (data layer): A record for www.example.com
import struct
tid = b'\\x12\\x34'
