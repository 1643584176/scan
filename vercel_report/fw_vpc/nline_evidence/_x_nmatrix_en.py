# -*- coding: utf-8 -*-
"""N-line matrix: full comparison of deniedCIDRs effective scope
Scenario A: deny 44.223.250.0/24 (httpbin.org's subnet, member of allowedDomain)
   A1 curl --resolve 44.223.250.126 -> ?
   A2 connect 44.223.250.126:443 -> ?
   A3 curl --resolve 3.234.68.252 (httpbin IP NOT denied) -> ? (expected 200)
   A4 connect 3.234.68.252:443 -> ?
   A5 connect 8.8.8.8:53 (not denied) -> ?
Scenario B: add deny 8.8.8.0/24
   B1 connect 8.8.8.8:53 -> ? (prev RC 0 = deny not enforced?)
   B2 connect 44.223.250.126:443 -> ? (still denied)
Scenario C: deny-all control
   C1 connect 3.234.68.252:443 -> ? (expected non-zero)
"""
...
# recover policy (previous run ended in deny-all)
set_policy({"mode": "custom", "allowedDomains": ["httpbin.org"]}, 'RECOVER')

# ===== Scenario A: deny 44.223.250.0/24 =====
