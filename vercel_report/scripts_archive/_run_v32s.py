...
NAME = 'v32s'
GUEST = 'vda32_celld_plugin_guest.py'
...
if __name__ == '__main__':
    ...
    if GUEST not in (r or ''):
        ...
        sys.exit(2)
    c, r = cmd(sid, 'sh', ['-c', 'nohup python3 /vercel/sandbox/' + GUEST + ' > /tmp/v32s_stdout.txt 2>&1 &'], timeout_ms=15000)
    ...
    for attempt in range(40):
        ...
        if attempt in (1, 5, 12) and c != 200:
            c2, r2 = cmd(sid, 'sh', ['-c', 'ls -la /vercel/sandbox/; tail -5 /tmp/v32s_stdout.txt 2>/dev/null'], timeout_ms=20000)
