...
def main():
    ...
    if GUEST not in (r or ''):
        ...
        sys.exit(2)
    c, r = cmdsh('nohup python3 /vercel/sandbox/%s > /tmp/v61_stdout.log 2>&1 &' % GUEST, 15000)
    ...
    c, r = api('POST', '/v2/sandboxes/sessions/%s/cmd?teamId=%s' % (sid, TEAM),
               {"command": "sh", "args": ["-c",
                "cat /vercel/sandbox/v61m.out 2>&1; echo ===COW===; ls -la /vercel/sandbox/ 2>&1; "
                "cat /vercel/sandbox/v61c2.out 2>&1"],
