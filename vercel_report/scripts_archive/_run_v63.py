...
def main():
    ...
    if GUEST not in (r or ''):
        ...
        sys.exit(2)
    c, r = cmdsh('nohup python3 /vercel/sandbox/%s > /tmp/v63_stdout.log 2>&1 &' % GUEST, 15000)
    ...
    c, r = api('POST', '/v2/sandboxes/sessions/%s/cmd?teamId=%s' % (sid, TEAM),
               {"command": "sh", "args": ["-c",
                "cat /vercel/sandbox/v63m.out 2>&1; echo ===COW===; ls -la /vercel/sandbox/ 2>&1; "
                "cat /vercel/sandbox/v63c.out 2>&1; echo ===SHARE===; ls -la /run/vercel/share/ 2>&1; "
                "cat /run/vercel/share/v63c2.out 2>&1"],
