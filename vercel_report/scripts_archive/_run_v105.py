...
NAME = 'v105'
GUEST = r'D:\scan\skills\non-traditional-vuln-hunting\vda105_guest.py'
PAY = r'D:\scan\skills\non-traditional-vuln-hunting\vda105_probe_guest.py'
...
def main():
    ...
    inject(sid, '/vercel/sandbox/vda105_guest.py', open(GUEST, 'rb').read())
    inject(sid, '/vercel/sandbox/vda105_probe_guest.py', open(PAY, 'rb').read())
    
    c, r = cmd(sid, 'python3', ['/vercel/sandbox/vda105_guest.py'], 300000)
    print('[run guest]', c)
    print((r or '')[:30000])

    c, r = cmd(sid, 'sh', ['-c', 'tail -c 30000 /vercel/sandbox/v105m.out 2>&1'], 20000)
    print('[guest tail]', c)
    print((r or '')[:26000])
