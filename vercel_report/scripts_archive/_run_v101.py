...
NAME = 'v101'
GUEST = r'D:\scan\skills\non-traditional-vuln-hunting\vda101_guest.py'
PAY = r'D:\scan\skills\non-traditional-vuln-hunting\vda101_probe_guest.py'
...
def main():
    ...
    inject(sid, '/vercel/sandbox/vda101_guest.py', open(GUEST, 'rb').read())
    inject(sid, '/vercel/sandbox/vda101_probe_guest.py', open(PAY, 'rb').read())
    
    c, r = cmd(sid, 'python3', ['/vercel/sandbox/vda101_guest.py'], 120000)
    print('[run guest]', c)
    print((r or '')[:30000])

    c, r = cmd(sid, 'sh', ['-c', 'tail -c 26000 /vercel/sandbox/v101m.out 2>&1'], 20000)
    print('[guest tail]', c)
    print((r or '')[:26000])
