# -*- coding: utf-8 -*-
"""v57 专用驱动: 容器 payload 诊断 (kill 前收集 ps/mountinfo/share)"""
import sys, os, time, json, base64
sys.path.insert(0, r'D:\scan\skills\non-traditional-vuln-hunting')
sys.stdout.reconfigure(encoding='utf-8')
from vercel_driver import api, TEAM, PROJ

GUEST = 'vda57_ctr_verify_guest.py'
...
def main():
    ...
    if GUEST not in (r or ''):
        ...
        sys.exit(2)
    c, r = cmdsh('nohup python3 /vercel/sandbox/%s > /tmp/v57_stdout.log 2>&1 &' % GUEST, 15000)
    ...
    c, r = api('POST', '/v2/sandboxes/sessions/%s/cmd?teamId=%s' % (sid, TEAM),
               {"command": "sh", "args": ["-c",
                "ls -la /vercel/sandbox/ 2>&1; echo ===M===; cat /vercel/sandbox/v57m.out 2>&1; "
                "echo ===SHARE===; ls -la /run/vercel/share/ 2>&1; cat /run/vercel/share/v57c.out 2>&1"],
