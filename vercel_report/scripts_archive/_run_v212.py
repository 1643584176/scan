# -*- coding: utf-8 -*-
"""v212 驱动: init.sock 服务枚举 + 签名跨路径测试"""
...
NAME = 'v212'
PAY = r'D:\scan\skills\non-traditional-vuln-hunting\vda212_probe_user.py'
...
def main():
    ...
    inject(sid, '/vercel/sandbox/vda212_probe_user.py', open(PAY, 'rb').read())

    c, r = cmd(sid, 'bash', ['-c', 'nohup python3 /vercel/sandbox/vda212_probe_user.py > /vercel/sandbox/v212out.log 2>&1 &'], 20000)
    print('[bg payload]', c, (r or '')[:200])
    time.sleep(5)

    # trigger 2 次 (hook 捕获)
    c, r = cmd(sid, 'echo', ['hello-v212a'], 30000)
    print('[trigger1]', c, (r or '')[:200])
    time.sleep(3)
    c, r = cmd(sid, 'echo', ['hello-v212b'], 30000)
    print('[trigger2]', c, (r or '')[:200])

    # 等 payload 完成 (探测 ~25s + hook 40s + 签名测试 ~15s)
    time.sleep(110)
    c, r = cmd(sid, 'cat', ['/vercel/sandbox/v212.log'], 20000)
    print('[v212 log]', c)
    print((r or '')[:20000])
