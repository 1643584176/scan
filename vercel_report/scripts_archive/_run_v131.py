...
NAME = 'v131'
GUEST = r'D:\scan\skills\non-traditional-vuln-hunting\vda131_guest.py'
PAY = r'D:\scan\skills\non-traditional-vuln-hunting\vda131_probe_guest.py'
...
def main():
    ...
    inject(sid, '/vercel/sandbox/vda131_guest.py', open(GUEST, 'rb').read())
    inject(sid, '/vercel/sandbox/vda131_probe_guest.py', open(PAY, 'rb').read())

    c, r = cmd(sid, 'python3', ['/vercel/sandbox/vda131_guest.py'], 300000)
    print('[run guest]', c)
    print((r or '')[:25000])

    c, r = cmd(sid, 'sh', ['-c', 'cat /vercel/sandbox/v131c.out 2>&1'], 20000)
    print('[v131c.out]', c)
    print((r or '')[:40000])
