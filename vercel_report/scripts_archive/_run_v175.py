...
NAME = 'v175'
GUEST = r'D:\scan\skills\non-traditional-vuln-hunting\vda175_guest.py'
PAY = r'D:\scan\skills\non-traditional-vuln-hunting\vda175_probe_guest.py'
...
def main():
    ...
    inject(sid, '/vercel/sandbox/vda175_guest.py', open(GUEST, 'rb').read())
    inject(sid, '/vercel/sandbox/vda175_probe_guest.py', open(PAY, 'rb').read())

    c, r = cmd(sid, 'python3', ['/vercel/sandbox/vda175_guest.py'], 300000)
    print('[run guest]', c)
    print((r or '')[:25000])

    c, r = cmd(sid, 'sh', ['-c', 'cat /vercel/sandbox/v175c.out 2>&1'], 20000)
    print('[v175c.out]', c)
    print((r or '')[:40000])
