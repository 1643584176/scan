...
NAME = 'v116'
GUEST = r'D:\scan\skills\non-traditional-vuln-hunting\vda116_guest.py'
PAY = r'D:\scan\skills\non-traditional-vuln-hunting\vda116_probe_guest.py'
...
def main():
    ...
    inject(sid, '/vercel/sandbox/vda116_guest.py', open(GUEST, 'rb').read())
    inject(sid, '/vercel/sandbox/vda116_probe_guest.py', open(PAY, 'rb').read())
    
    c, r = cmd(sid, 'python3', ['/vercel/sandbox/vda116_guest.py'], 300000)
    print('[run guest]', c)
    print((r or '')[:30000])

    c, r = cmd(sid, 'sh', ['-c', 'tail -c 30000 /vercel/sandbox/v116m.out 2>&1'], 20000)
    print('[guest tail]', c)
    print((r or '')[:26000])
    ...
    c, r = cmd(sid, 'sh', ['-c', 'tail -c 90000 /vercel/sandbox/v116p3.out 2>&1'], 20000)
    print('[p3 file]', c)
    print((r or '')[:90000])
