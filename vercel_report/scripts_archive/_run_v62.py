# -*- coding: utf-8 -*-
"""v61 专用快速驱动: rbind /run + 诊断链 + COW 写验证 (单次跑完)"""
import sys, os, time, json, base64
sys.path.insert(0, r'D:\scan\skills\non-traditional-vuln-hunting')
sys.stdout.reconfigure(encoding='utf-8')
from vercel_driver import api, TEAM, PROJ

GUEST = 'vda62_chroot_guest.py'
...
def mk():
    ...
    if c == 402:
        print('[402] cleaning snapshots and retry')
        clean_snaps()
        time.sleep(1)
        c, r = api('POST', '/v2/sandboxes?teamId=%s' % TEAM, body, timeout=120)
        print('[create retry]', c, r[:200])
    if c != 200:
...
def main():
    ...
    if GUEST not in (r or ''):
        ...
        sys.exit(2)
    c, r = cmdsh('nohup python3 /vercel/sandbox/%s > /tmp/v62_stdout.log 2>&1 &' % GUEST, 15000)
    ...
    c, r = api('POST', '/v2/sandboxes/sessions/%s/cmd?teamId=%s' % (sid, TEAM),
               {"command": "sh", "args": ["-c",
                "cat /vercel/sandbox/v62m.out 2>&1; echo ===COW===; ls -la /vercel/sandbox/ 2>&1; "
                "cat /vercel/sandbox/v62c.out 2>&1; echo ===SHARE===; ls -la /run/vercel/share/ 2>&1; "
                "cat /run/vercel/share/v62c2.out 2>&1"],
