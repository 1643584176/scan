# -*- coding: utf-8 -*-
"""v90 驱动: 注入 exec_probe.sh + cell API drive 容器 Exec 侦察"""
...
NAME = 'v90'
GUEST = r'D:\scan\skills\non-traditional-vuln-hunting\vda90_guest.py'
PAY = r'D:\scan\skills\non-traditional-vuln-hunting\vda90_probe_guest.py'
...
def main():
    ...
    inject(sid, '/vercel/sandbox/vda90_guest.py', open(GUEST, 'rb').read())
    inject(sid, '/vercel/sandbox/vda90_probe_guest.py', open(PAY, 'rb').read())
    time.sleep(1)

    c, r = cmd(sid, 'python3', ['/vercel/sandbox/vda90_guest.py'], 120000)
    print('[run guest]', c)
    print((r or '')[:30000])

    c, r = cmd(sid, 'sh', ['-c', 'tail -c 26000 /vercel/sandbox/v90m.out 2>&1'], 20000)
    print('[guest tail]', c)
    print((r or '')[:26000])
