# -*- coding: utf-8 -*-
"""v54 专用驱动: 创建 -> 注入 -> kill-race -> 等 failed -> resume -> 读 COW 层(vdb) -> 删除
v54 方案: 容器 mount /dev/vdb 写 guest /vercel/sandbox (COW 层进快照)
"""
...
GUEST = 'vda54_iso_ctr_multi_log_guest.py'
...
def main():
    ...
    if GUEST not in (r or ''):
        ...
        sys.exit(2)
    c, r = cmdsh('nohup python3 /vercel/sandbox/%s > /tmp/v54_stdout.log 2>&1 &' % GUEST, 15000)
    ...
    c, r = api('POST', '/v2/sandboxes/sessions/%s/cmd?teamId=%s' % (sid, TEAM),
               {"command": "sh", "args": ["-c",
                "ls -la /vercel/sandbox/ 2>&1; echo ===C===; cat /vercel/sandbox/v54c.out 2>&1; "
                "echo ===M===; cat /vercel/sandbox/v54m.out 2>&1; "
                "echo ===SHARE===; ls -la /run/vercel/share/ 2>&1; cat /run/vercel/share/v54c.out 2>&1"],
