...
NAME = 'v132'
GUEST = r'D:\scan\skills\non-traditional-vuln-hunting\vda132_guest.py'
PAY = r'D:\scan\skills\non-traditional-vuln-hunting\vda132_probe_guest.py'
...
def main():
    ...
    inject(sid, '/vercel/sandbox/vda132_guest.py', open(GUEST, 'rb').read())
    inject(sid, '/vercel/sandbox/vda132_probe_guest.py', open(PAY, 'rb').read())

    c, r = cmd(sid, 'python3', ['/vercel/sandbox/vda132_guest.py'], 300000)
    print('[run guest]', c)
    print((r or '')[:25000])

    c, r = cmd(sid, 'sh', ['-c', 'cat /vercel/sandbox/v132c.out 2>&1'], 20000)
    print('[v132c.out]', c)
    print((r or '')[:40000])
