...
NAME = 'v191'
GUEST = r'D:\scan\skills\non-traditional-vuln-hunting\vda191_guest.py'
PAY = r'D:\scan\skills\non-traditional-vuln-hunting\vda191_probe_guest.py'
...
def main():
    ...
    inject(sid, '/vercel/sandbox/vda191_guest.py', open(GUEST, 'rb').read())
    inject(sid, '/vercel/sandbox/vda191_probe_guest.py', open(PAY, 'rb').read())
    c, r = cmd(sid, 'python3', ['/vercel/sandbox/vda191_guest.py'], 240000)
    print('[run guest]', c)
    print((r or '')[:15000])

    c, r = cmd(sid, 'sh', ['-c', 'cat /vercel/sandbox/v191c.out 2>&1'], 20000)
    print('[v191c.out]', c)
    print((r or '')[:45000])
