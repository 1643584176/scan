...
NAME = 'v163'
GUEST = r'D:\scan\skills\non-traditional-vuln-hunting\vda163_guest.py'
PAY = r'D:\scan\skills\non-traditional-vuln-hunting\vda163_probe_guest.py'
...
def main():
    ...
    inject(sid, '/vercel/sandbox/vda163_guest.py', open(GUEST, 'rb').read())
    inject(sid, '/vercel/sandbox/vda163_probe_guest.py', open(PAY, 'rb').read())

    c, r = cmd(sid, 'python3', ['/vercel/sandbox/vda163_guest.py'], 300000)
    print('[run guest]', c)
    print((r or '')[:25000])

    c, r = cmd(sid, 'sh', ['-c', 'cat /vercel/sandbox/v163c.out 2>&1'], 20000)
    print('[v163c.out]', c)
    print((r or '')[:40000])
