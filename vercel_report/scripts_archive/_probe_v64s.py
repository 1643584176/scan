# -*- coding: utf-8 -*-
"""v64s 驱动: inject _cell_dump.py 到沙箱执行 (celld 字符串分析)"""
...
def main():
    ...
    time.sleep(2)
    src = open(DUMPS, 'rb').read()
    b64 = base64.b64encode(src).decode()
    inject = "import base64;open('/vercel/sandbox/cd.py','wb').write(base64.b64decode('%s'))" % b64
    ...
    print('[inject]', c)
    time.sleep(1)
    s = 'mkdir -p /mnt/vdax; sudo mount /dev/vda /mnt/vdax 2>&1; python3 /vercel/sandbox/cd.py'
    c, r = api('POST', '/v2/sandboxes/sessions/%s/cmd?teamId=%s' % (sid, TEAM),
