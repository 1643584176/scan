# -*- coding: utf-8 -*-
"""v82 驱动: 注入 exec_probe.sh + cell API drive 容器 Exec 侦察"""
...
NAME = 'v82'
GUEST = r'D:\scan\skills\non-traditional-vuln-hunting\vda82_guest.py'
PAY = r'D:\scan\skills\non-traditional-vuln-hunting\vda82_probe_guest.py'
...
def main():
    ...
    inject(sid, '/vercel/sandbox/vda82_guest.py', open(GUEST, 'rb').read())
    inject(sid, '/vercel/sandbox/vda82_probe_guest.py', open(PAY, 'rb').read())
    time.sleep(1)

    c, r = cmd(sid, 'python3', ['/vercel/sandbox/vda82_guest.py'], 120000)
    print('[run guest]', c)
    print((r or '')[:30000])
