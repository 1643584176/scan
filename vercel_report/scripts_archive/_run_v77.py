# -*- coding: utf-8 -*-
"""v77 驱动: 注入 exec_probe.sh + cell API drive 容器 Exec 侦察"""
...
NAME = 'v77'
GUEST = r'D:\scan\skills\non-traditional-vuln-hunting\vda77_guest.py'
PAY = r'D:\scan\skills\non-traditional-vuln-hunting\vda77_probe_guest.py'
...
def main():
    ...
    inject(sid, '/vercel/sandbox/vda77_guest.py', open(GUEST, 'rb').read())
    inject(sid, '/vercel/sandbox/vda77_probe_guest.py', open(PAY, 'rb').read())
    time.sleep(1)

    c, r = cmd(sid, 'python3', ['/vercel/sandbox/vda77_guest.py'], 120000)
    print('[run guest]', c)
    print((r or '')[:30000])
