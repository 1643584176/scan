# -*- coding: utf-8 -*-
"""v76 驱动: 注入 exec_probe.sh + cell API drive 容器 Exec 侦察"""
...
NAME = 'v76'
GUEST = r'D:\scan\skills\non-traditional-vuln-hunting\vda76_guest.py'
PAY = r'D:\scan\skills\non-traditional-vuln-hunting\vda76_probe_guest.py'
...
def main():
    ...
    inject(sid, '/vercel/sandbox/vda76_guest.py', open(GUEST, 'rb').read())
    inject(sid, '/vercel/sandbox/vda76_probe_guest.py', open(PAY, 'rb').read())
    time.sleep(1)

    c, r = cmd(sid, 'python3', ['/vercel/sandbox/vda76_guest.py'], 120000)
    print('[run guest]', c)
    print((r or '')[:30000])
