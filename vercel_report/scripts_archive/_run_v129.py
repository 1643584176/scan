...
NAME = 'v129'
GUEST = r'D:\scan\skills\non-traditional-vuln-hunting\vda129_guest.py'
PAY = r'D:\scan\skills\non-traditional-vuln-hunting\vda129_probe_guest.py'
...
def main():
    ...
    inject(sid, '/vercel/sandbox/vda129_guest.py', open(GUEST, 'rb').read())
    inject(sid, '/vercel/sandbox/vda129_probe_guest.py', open(PAY, 'rb').read())

    c, r = cmd(sid, 'python3', ['/vercel/sandbox/vda129_guest.py'], 300000)
    print('[run guest]', c)
    print((r or '')[:25000])

    c, r = cmd(sid, 'sh', ['-c', 'cat /vercel/sandbox/v129c.out 2>&1'], 20000)
    print('[v129c.out]', c)
    print((r or '')[:40000])
