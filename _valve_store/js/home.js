...
GHomepage = {
	...
	InitUserData: function( rgParams )
	{
		try {
			...
			GHomepage.bOptedIntoHomeBeta = rgParams.bOptedIntoHomeBeta || false;
		...
		else
		{
			...
			if ( strImageURL )
			{
				$ImgCtn.append( $J('<div/>', {'class': 'cluster_maincap_fill ' + (rgItemData.package_header ? 'package' : '') } )
					.append(
						$J('<img/>', {'class': 'cluster_maincap_fill_placeholder', src: 'https://store.st.dl.eccdnx.com/public/images/v6/home/maincap_placeholder_616x353.gif' } ),
						$J('<img/>', {'class': 'cluster_capsule_image cluster_maincap_fill_bg', src: 'https://store.st.dl.eccdnx.com/public/images/blank.gif', 'data-image-url': strImageURL } ),
						$J('<img/>', {'class': 'cluster_maincap_fill_header', src: 'https://store.st.dl.eccdnx.com/public/images/blank.gif', 'data-image-url': strImageURL } )
					)
				);
			}
			else
	...
	BuildMainCapsuleItemv2: function( rgItem, strFeatureContext, nDepth )
	{
		...
		else
		{
			...
			else if ( rgRecommendationReasons.top_seller )
			{
				...
				$TopSellerReason.append( $J( '<img/>', { src: 'https://media.st.dl.eccdnx.com/store//icons/rank_icon_lg.svg' } ) );
	...
	RenderRecommendedCreatorApps: function()
	{
		...
		const fnBuildCreatorArea = function( oItem, nDepth )
		{
			...
			const fnUpdateFollowStatus = function( $elFollowButton, bFollowing )
			{
				if ( bFollowing )
				{
					$elFollowButton.html( '<span><img src="https://store.st.dl.eccdnx.com/public/images/v6/ico/ico_selected.png">Following</span>' );
	...
	RenderContentHubCarousel: function()
	{
		...
		GHomepage.FillPagedCapsuleCarousel( GHomepage.rgContentHubs, $contentHubCarousel,
			function( item, strFeature, rgOptions, nDepth )
			{
				...
				var $image = $J( '<img/>', {
					'src': "https://store.st.dl.eccdnx.com/public/images/blank.gif",
					"alt": '',	// intentionally blank, there is text overlaid on top of the image
					"data-image-url": "https://store.st.dl.eccdnx.com/" + "categories/homepageimage/" + strImageURL + "?cc=" + cc + "&l=" + l + "&v=2",
				} );
			...
			if ( bGamepad )
			{
				...
				$SeeMore.append( `
					<div class="see_more_gamepad_hint">
						<img src="https://store.st.dl.eccdnx.com/public/images/ico_gamepad/shared_button_y.svg">
						<div>See More</div>
					</div>
				`);
				$Ctn.append( $SeeMore );
	...
	BuildHomePageCapsule: function( item, strFeatureContext, rgCapsuleOptions, nDepth )
	{
		...
		if ( rgOptions.disable_autosizer )
		{
			if ( rgOptions.lazy )
			{
				...
				let src = 'https://store.st.dl.eccdnx.com/public/images/v6/home/maincap_placeholder_616x353.gif';
				if ( rgOptions.capsule_size === 'header' )
					src = 'https://store.st.dl.eccdnx.com/public/images/v6/home/header_placeholder_460x215.gif';
		...
		else
		{
			...
			else
			{
				...
				else
				{
					rgImageProperties = {
						'class': 'sale_capsule_image autosize',
						'src': 'https://store.st.dl.eccdnx.com/public/images/v6/home/maincap_placeholder_616x353.gif',
	...
	BuildCommunityRecommendationCapV2: function( strFeatureContext, oItem )
	{
		...
		var $Image = $J('<img/>', { src: 'https://store.st.dl.eccdnx.com/public/images/blank.gif', 'data-image-url': rgItemData.header, 'data-image2x-url': rgItemData.header_2x ?? '',  alt: rgItemData.name } );
...
{
    return 'https://avatars.st.dl.eccdnx.com/' + strAvatarHash + ( sizeStr || '' ) + '.jpg';
}

function GetScreenshotURL( appid, filename, sizeStr )
{
	if( sizeStr )
		return 'https://shared.st.dl.eccdnx.com/store_item_assets/steam/' + 'apps/' + appid + '/' + filename.replace('.jpg', sizeStr + '.jpg');

	return 'https://shared.st.dl.eccdnx.com/store_item_assets/steam/' + 'apps/' + appid + '/' + filename;
}
...
GSteamCurators = {
    ...
    BuildResponsiveHomePageGiantCap: function( strFeatureContext, oItem )
	{
        ...
        var $Thumb = $J('<img/>', {'class': 'recommendation_thumb', src: 'https://store.st.dl.eccdnx.com/public/images/recommended_thumb.png', 'alt': ''});
