...
# ---- 本地 s_server ----
print(run("pkill -f 's_serve[r]'; openssl req -x509 -newkey rsa:2048 -keyout /tmp/k.pem -out /tmp/c.pem -days 1 -nodes -subj '/CN=example.com' 2>/dev/null; openssl s_server -accept 127.0.0.1:4443 -cert /tmp/c.pem -key /tmp/k.pem -msg > /tmp/srv.log 2>&1 &"))
time.sleep(1.5)

# ---- 1. 我的 CH 发往本地 s_server ----
...
try:
    ...
    print("   resp:", resp[:60].hex() if resp else "EMPTY")
except Exception as e:
...
# ---- 2. openssl 标准 CH 发往本地 s_server(对照) ----
print("== [2] openssl CH -> local s_server ==")
print(run("echo | timeout 8 openssl s_client -connect 127.0.0.1:4443 -servername example.com -msg 2>&1 | head -50")[-1800:])
print("--- srv.log after openssl ---")
print(run("cat /tmp/srv.log")[-800:])
