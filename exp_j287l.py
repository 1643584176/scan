# -*- coding: utf-8 -*-
"""实验J287l: 分段定位 RUN 无输出 - syscall枚举 vs IMDS, 逐步print观察停在哪"""
...
# 0) 验证: 进程被 kill 后, 已 flush 的输出是否仍通过 cmd 通道返回
out = run_cmd(sid, "import os;print('A_FLUSHED',flush=True);os.kill(os.getpid(),9);print('B_NEVER')", "KILL_TEST", timeout=100)
print("KILL_TEST out:", repr(out[:200]), flush=True)
...
print("=== PHASE1 syscall enum ===", flush=True)
for n in sorted(SYS):
    if n in SKIP:
        continue
    try:
        e = probe(n)
        print("S%d:%s e=%d" % (n, SYS[n], e), flush=True)
    except Exception as ex:
        print("S%d:%s EXC %s" % (n, SYS[n], ex), flush=True)
...
ok = upload_file(sid, "/tmp/rec2.py", PAYLOAD.encode())
print("upload:", ok, flush=True)

out = run_cmd(sid, "exec(open('/tmp/rec2.py').read())", "EXEC_DIRECT", timeout=280)
print("EXEC_DIRECT out len:", len(out or ""), flush=True)
print((out or "")[:10000], flush=True)
