# -*- coding: utf-8 -*-
"""输出 scan3 命中文件的命中行上下文,用于甄别真伪(只读)。"""
import io, os, re, sys
...
TXT_EXT = {'.py', '.json', '.txt', '.md', '.js', '.ts', '.env', '.yaml', '.yml', '.toml', '.xml', '.html', '.csv', '.sql', '.cfg', '.conf', '.ini', '.go', '.rs', '.tsx', '.jsx'}
SKIP_DIR = {'node_modules', '.git', '.venv', '__pycache__', '_js', '_sb_js', '_kiwi_ref', '_kiwi_work', '_pg_session_jwt_src', '_gotrue_src', '_pgrepack_src', '_openapi'}
...
for rel in sorted(ls):
    ...
    for name, rx in PATS:
        for i, ln in enumerate(lines):
            if rx.search(ln):
                found.append('  [%s L%d] %s' % (name, i + 1, ln.strip()[:170]))
                break
    if found:
        print('###', rel)
        for f_ in found:
