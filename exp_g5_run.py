# 实验G5驱动: OIDC token API 接受性验证
# create(custom allowedDomains=[httpbin.org, api.vercel.com])
# update: allow.api.vercel.com -> [{match:{path:{startsWith:"/v2"}}, forwardURL:"https://api.vercel.com"}]
# sandbox 请求 api.vercel.com/v2/* -> 转发 + OIDC token(aud=api.vercel.com) -> API 是否接受?
...
c, r = api("POST", f"/v2/sandboxes/sessions/{sid}/network-policy?teamId={TEAM}",
           {"allow": {"api.vercel.com": [
               {"match": {"path": {"startsWith": "/v2"}},
                "forwardURL": "https://api.vercel.com"}]}})
print("update:", c, r[:200])

payload = base64.b64encode(pathlib.Path("exp_g5.py").read_bytes()).decode()
