# 本地流程: 创建沙箱 -> update policy(allow风格 match+forwardURL) -> 沙箱内请求矩阵
...
# update policy: allow 风格 + match + forwardURL
c, r = api("POST", f"/v2/sandboxes/sessions/{sid}/network-policy?teamId={TEAM}",
           {"allow": {"httpbin.org": [
               {"match": {"path": {"exact": "/anything"}},
                "forwardURL": "https://httpbin.org/anything"}]}})
print("update:", c, r[:300])

payload = base64.b64encode(pathlib.Path("exp_e1.py").read_bytes()).decode()
