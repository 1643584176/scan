# -*- coding: utf-8 -*-
"""vda60: rbind /run 方案 —— 容器 rootfs 必含 /run 目录, rbind 宿主 /run 后
容器内 /run/cell/cell.sock 与 /run/vercel/share 直接可达 (v59 错误: bind source 需在容器 rootfs 存在)
"""
import os, time, socket, ctypes, re, struct, subprocess, json

OUT = '/vercel/sandbox/v60m.out'
os.makedirs('/vercel/sandbox', exist_ok=True)
f = open(OUT, 'w', encoding='utf-8', errors='replace')
EXTRA = '/mnt/vdax/root/v60m.out'
...
def main():
    ...
    log('cell check: %s' % os.path.exists('/run/cell/cell.sock'))
    log('share check: %s' % os.path.isdir('/run/vercel/share'))
    try:
        log('share ls: %s' % (sorted(os.listdir('/run/vercel/share')) if os.path.isdir('/run/vercel/share') else 'N/A'))
    except Exception as e:
...
def main():
    ...
    spec = {
        ...
        "process": {
            "user": {"uid": 0, "gid": 0},
            "args": ["/bin/sh", "-c",
                     "python3 /run/vercel/share/v60_payload.py; sleep 99999"],
    ...
    while t_wait < 35:
        ...
        try:
            ...
            if cur != seen:
                seen = cur
                log('--- share v60c.out @%ds ---\n%s' % (t_wait, cur[-2500:]))
            if 'V60C_DONE' in cur or 'DRIVE FREE' in cur:
