# -*- coding: utf-8 -*-
"""v118 payload: StreamOutput 修正帧格式
Connect flags: 0x01=COMPRESSED 0x02=END_STREAM (v117 错误破译)
请求: DATA帧(0x00+len+proto) + END_STREAM(0x02+0)
响应: chunked + envelope 帧流
输出 /vercel/sandbox/v118c.out"""
import socket, struct, time, signal, json

OUT = '/vercel/sandbox/v118c.out'
...
def stream_bidi(sockpath, path, proto_body, t=15.0):
    """正确 bidi: DATA帧(0x00+len+body) + END_STREAM(0x02+0)"""
    try:
        s = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
        s.settimeout(t)
        s.connect(sockpath)
        env1 = b'\x00' + struct.pack('>I', len(proto_body)) + proto_body
        ...
        body = dechunk(body)
        # 解析 envelope 帧: fl&0x01=COMPRESSED fl&0x02=END_STREAM
        out = b''
        frames = []
        off = 0
        while off + 5 <= len(body):
            ...
            if fl & 0x02:
                ...
                out += pay
            off += 5 + ln
        log('STREAM %s -> %s dechunked=%dB frames=%s out=%r' % (path, status, len(body), frames, out[:800]))
        return status, out
    except Exception as e:
...
# P2 StreamOutput bidi 正确帧 (DATA 0x00 + END_STREAM 0x02)
...
if pid:
    ...
    if out:
        log('OUT text: %r' % out[:600])
        # 尝试 proto 解析 stdout 字段
        try:
            # StreamOutputResponse 未知字段号, 尝试提取可打印文本
            txt = ''.join(chr(b) for b in out if 32 <= b < 127 or b in (10, 13))
            log('OUT ascii: %r' % txt[:500])
        except Exception:
            pass
