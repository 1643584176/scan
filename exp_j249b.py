# 实验J249b: 确认 /dev/vda 内容 - 读偏移找宿主文件 + XFS 结构解析 (修复引号嵌套)
...
code = (
    ...
    "    if len(cur) >= 6:\n"
    "        frags.append(cur)\n"
    "    print('OFF', hex(off), 'got', len(d), 'frags', len(frags), flush=True)\n"
    "    for f in frags[:6]:\n"
...
try:
    ...
    p("MULTI_READ", "rc", r.returncode, "OUT", (r.stdout + r.stderr)[:2500].replace(chr(10), "|"))
except Exception as e:
...
code = (
    ...
    "print('XFS magic', magic, 'bsize', bsize, 'dblocks', dblocks, 'agcount', agcount, 'agsize', agsize, flush=True)\n"
    "print('FS_SIZE', dblocks * bsize, 'bytes', flush=True)\n"
    ...
    "    if len(cur) >= 8:\n"
    "        frags.append(cur)\n"
    "    print('AG', ag, 'base', hex(base), 'frags', len(frags), flush=True)\n"
    "    for f in frags[:8]:\n"
...
code = (
    ...
    "    off += step\n"
    "print('SAMPLED', total, 'bytes', flush=True)\n"
    "for pat, lst in hits.items():\n"
    "    print('HIT', pat, len(lst), flush=True)\n"
    "    for o, s in lst[:5]:\n"
    "        print('   ', o, s[:120], flush=True)\n"
    "if not hits:\n"
