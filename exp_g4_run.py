# 实验G4驱动: forwardURL 匹配机制 + OIDC token 附加验证
# create(custom allowedDomains=[httpbin.org, api.vercel.com])
# update: allow.api.vercel.com -> [{match:{path:{startsWith:"/v2"}}, forwardURL:"https://httpbin.org/anything"}]
# sandbox 请求 https://api.vercel.com/v2/user -> 若转发生效, httpbin 回显附加头(OIDC token)
...
payload = base64.b64encode(pathlib.Path("exp_g4.py").read_bytes()).decode()
