# -*- coding: utf-8 -*-
# _ar_recon_out.py - fetch session result of recon run
...
if isinstance(d, list) and d:
    sess = d[0]
    print("session state:", sess.get("state"), "steps:", sess.get("steps_count"))
    print("usage:", json.dumps(sess.get("usage"), ensure_ascii=False)[:400])
    ...
    if isinstance(res, dict):
        ...
        for k in ("state", "final_report", "error", "summary"):
            if k in res:
                print("\n### result.%s:" % k)
                v = res[k]
                print(json.dumps(v, ensure_ascii=False)[:6000] if not isinstance(v, str) else v[:6000])
        # agent messages / transcript
        for k2 in ("messages", "transcript", "steps", "events"):
            if k2 in res and res[k2]:
                print("\n### result.%s (len %d):" % (k2, len(res[k2])))
                print(json.dumps(res[k2], ensure_ascii=False)[:8000])
    else:
        print("result:", str(res)[:4000])
    # interactions final state
    it = sess.get("interactions")
    if it:
        print("\ninteractions final:", json.dumps(it, ensure_ascii=False)[:2000])
else:
