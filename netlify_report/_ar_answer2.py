# -*- coding: utf-8 -*-
# _ar_answer2.py - probe answer payload with refId
...
# try: refId = interaction id, answer as option_index
for body in [
    {"refId": IID, "option_index": 0},
    {"refId": IID, "answer": {"option_index": 1}},
    {"refId": IID, "answer": "No, that recon was actually what I needed"},
    {"refId": IID, "choice": 1},
    {"refId": IID, "payload": {"option_index": 1}},
]:
    s, d = req("POST", API + "/agent_runners/%s/sessions/%s/answers" % (RID, SID), tok=TOKEN_B, body=body)
    print(json.dumps(body, ensure_ascii=False)[:100], "->", s, json.dumps(d, ensure_ascii=False)[:300] if not isinstance(d, str) else d)
