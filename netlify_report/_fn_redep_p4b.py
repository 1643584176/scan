...
s, raw = api('/api/v1/sites/%s/deploys' % SITE_B, method='POST',
             body={'title': 'fn-p4c', 'files': files, 'functions': functions})
print('create:', s, raw[:150].decode('utf-8', 'replace'))
d = json.loads(raw)
DID = d.get('id')
...
for i in range(12):
    ...
    print('PUT fn try%d: %d %s' % (i, s2, raw2[:150].decode('utf-8', 'replace').replace('\n', ' ')))
    if s2 == 200:
        ok = True
        break
    time.sleep(1.0)
...
print('put html:', s, raw[:150].decode('utf-8', 'replace'))
